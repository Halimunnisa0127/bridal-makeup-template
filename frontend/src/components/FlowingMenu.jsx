...
function FlowingMenu({
  ...
  return (
    <div className="w-full h-full overflow-hidden" style={{ backgroundColor: bgColor }}>
      <nav className="flex flex-col h-full m-0 p-0 justify-center">
        {items.map((item, idx) => (
          <MenuItem
            key={idx}
            {...item}
...
function MenuItem({ link, text, image, speed, textColor, marqueeBgColor, marqueeTextColor, borderColor, isFirst }) {
  ...
  return (
    <div
      className="flex-1 relative overflow-hidden text-center max-h-[12vh]"
      ref={itemRef}
        ...
        <div className="h-full w-fit flex" ref={marqueeInnerRef}>
          {[...Array(repetitions)].map((_, idx) => (
            <div className="marquee-part flex items-center flex-shrink-0" key={idx} style={{ color: marqueeTextColor }}>
              ...
              <div
                className="w-[150px] md:w-[200px] h-[5vh] md:h-[7vh] my-[2em] mx-[2vw] py-[1em] rounded-[50px] bg-cover bg-center"
